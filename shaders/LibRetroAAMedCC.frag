...
const float Saturation = 1.32; // Adjust saturation (range: 0.0 to 2.0)
...
void main() {
    ...
    vec4 xh = yx * vec4(0.0, 1.5, 0.0, 1.5); // 4.0
    vec4 yv = yx * vec4(1.5, 0.0, 1.5, 0.0);
    ...
    // aa output
   vec4 finalColor = mix(firstPassColor, secondPassColor, 0.55);

    // color correction
    finalColor.rgb = ((finalColor.rgb - 0.5) * max(Contrast, 0.0)) + 0.5;
    finalColor.rgb += Brightness;
    float intensity = dot(finalColor.rgb, vec3(0.2125, 0.7154, 0.0721));
    finalColor.rgb = mix(vec3(intensity), finalColor.rgb, Saturation);
    finalColor.r *= RedAdjustment;
    finalColor.g *= GreenAdjustment;
    finalColor.b *= BlueAdjustment;

    // output overall with color correction
